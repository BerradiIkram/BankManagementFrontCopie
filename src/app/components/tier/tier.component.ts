...
export class TierComponent {
  ...
  handleDeletePersonneMorale(personnemorale : Personnemorale){
...
error : err => console.log(err)
  ...
  handleDeletePersonnePhysique(personnephysique : Personnephysique){
    console.log(personnephysique);
    let conf = confirm("Are you sure ?");
    if (!conf)
      return;
    console.log(this.tierService);
    this.tierService.deletPersonneMorale(personnephysique.idClient).subscribe({
      next :(resp )=> {

        console.log(personnephysique.idClient);
        this.personnephysique = this.personnephysique.pipe(
          map(data=> {
            let index = data.indexOf(personnephysique);
            data.slice(index,1);
            return data;
          })
        )},
